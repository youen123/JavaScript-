...
- async 不会阻塞网页，边下载边往下解析，不保证顺序，谁先下完先执行
- defer 边下边解析，等到文档完全被解析和显示之后再执行，保证顺序

##### JavaScript变量类型
###### 数据类型：

* undefined 未被初始化的变量，直接使用会出错，除了typeof
* null 空对象指针  
* Number // 引出-->特殊值NaN
* String
* Boolean 
* 复杂数据类型 object
* 基本类型保存在栈内存中；引用类型的值为对象，保存在堆内存中，包含引用类型值的变量实际上包含的并不是对象本身，而是一个指向该对象的指针
...
- Number() 适用于任何数据类型，如果是对象，调用valueof方法，如果转换的结果是NaN，调用对象的toString方法
- parseInt()/parseFloat()  针对字符串
...
###### 函数
 - arguments对象   
  是一个类数组对象（可用下标的方式去访问其中的元素），arguments的值与对应命名参数的值保持同步，但不是访问相同的内存空间。
 - 返回值
...
####  执行环境和作用域
###### 执行环境   
 分为全局执行环境和函数执行环境。每个函数都有自己的执行环境。执行环境定义了函数可以访问的数据。每进入一个执行环境，会创建一个作用域链。执行环境有助于确定何时释放内存。
